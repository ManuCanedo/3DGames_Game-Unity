function SessionData=EditSession(SessionRecordings)
...
t = uitable('Units','normalized','Parent', f, 'Position', [0 0 1 1]);
...
aux=struct2cell(SessionRecordings);
%names=extractfield(SessionRecordings,'name')';
names=aux(1,:);
select=false(length(names),1);
Contents(1:length(names))={'UNK'};
set(t,'Data',[cellstr(names'),num2cell(select),Contents']);
set(t, 'CellEditCallback', @EditTable);

SessionData.Name=answer;
